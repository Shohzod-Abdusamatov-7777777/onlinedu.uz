                    ...
                    <div className="course">
                        <div className="course_inner">
                            ...
                            <div className="p-2 py-3 course_info">
                                ...
                                >
                                    {course.category &&
                                        course.category.translations.filter((lng) => lng.locale === "uz")[0]
                                            .value}
                                </span>
                                <div className="d-inline-block align-middle" style={{ whiteSpace: "nowrap" }}>
                                    {course.rating !== null ? (
                                        <>
                                            {_.range(Math.floor(parseFloat(course.rating))).map(
                                                (star, index) => (
                                                    <span key={index}>
                                            ...
                                            {/* empty star */}
                                            {_.range(4 - Math.floor(parseFloat(course.rating))).map(
                                                (star, index) => (
                                                    <span key={index}>
                                                        <BsStar className="text-yellow ml-1" />
                                                    </span>
                                            ...
                                            {/* end star */}{" "}
                                            <span className="mt-1 pl-2 text-yellow">{course.rating}</span>
                                        </>
                                    ) : (
                                        <>
